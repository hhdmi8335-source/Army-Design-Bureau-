...
import { analyzeVideoFrames as geminiAnalyzeVideo, AnalysisResult, AnalysisReport, Detection } from "@/services/geminiAnalysis";
...
const VideoDetection = ({ onReportGenerated }: VideoDetectionProps) => {
  ...
  const analyzeFrame = useCallback(async (frameDataUrl: string): Promise<{ detections: Detection[]; scene_summary: string }> => {
    try {
      const data = await geminiAnalyzeVideo([frameDataUrl], 'detect') as AnalysisResult;
  ...
  const startAnalysis = async () => {
    ...
    for (let i = 0; i < totalFrames; i++) {
      ...
      // Simulate tracking IDs across frames
      const trackedDetections = result.detections.map((det, index) => ({
        ...det,
        tracking_id: det.tracking_id || `V-${(index + 1).toString().padStart(3, '0')}`
      }));

      setCurrentDetections(trackedDetections);
    ...
    // Generate a final summary report for the video
    let finalReport = null;
    try {
      const keyFrames = frameDetections
        .filter(f => f.detections.length > 0)
        .slice(0, 6);
      const framesToSend = keyFrames.length > 0 
        ? keyFrames 
        : frameDetections.filter((_, i) => i % Math.ceil(frameDetections.length / 4) === 0).slice(0, 4);
      
      const fullData = await geminiAnalyzeVideo(framesToSend.map(f => f.frameDataUrl), 'full') as AnalysisResult;
      finalReport = fullData.report;
    } catch (e) {
    ...
    if (isPlaceholder) {
      ...
      toast({ title: "Analysis Complete", description: `Analyzed ${frameDetections.length} frames. (Database not configured)` });
    } else {
      try {
        const { error: logError } = await supabase.from('detection_logs').insert([{
          ...
          metadata: JSON.parse(JSON.stringify({ 
            total_frames_analyzed: frameDetections.length, 
        ...
        if (logError) {
          ...
          toast({ 
            title: "Log Failed", 
            description: `Failed to save video analysis: ${logError.message}`, 
            variant: "destructive" 
          });
        }
      } catch (e) {
        ...
        toast({ 
          title: "Log Error", 
          description: "An unexpected error occurred while saving video analysis.", 
          variant: "destructive" 
        });
      }
    }
  ...
  const generateReport = async () => {
    ...
    try {
      const data = await geminiAnalyzeVideo(framesToSend.map(f => f.frameDataUrl), 'report') as AnalysisReport;
      onReportGenerated(data, framesToSend.map(f => f.frameDataUrl));
                ...
                currentDetections.map((det, i) => (
                  <div key={i} className="p-3 bg-tactical-dark rounded border border-border">
                    ...
                    {det.reliability_level && (
                      <div className={`text-[10px] font-mono font-bold uppercase p-1 rounded text-center ${
