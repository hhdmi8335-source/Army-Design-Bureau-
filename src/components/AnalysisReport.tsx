                  ...
                  <div className="grid md:grid-cols-5 gap-0">
                    ...
                    <div className="md:col-span-3 p-5">
                      ...
                      <div className="grid grid-cols-2 gap-2 mb-3">
                        {finding.speed && (
                          <div className="bg-black/20 p-1.5 rounded text-[10px] font-mono border border-white/5">
                            <span className="text-muted-foreground uppercase">Speed:</span> {finding.speed}
                          </div>
                        )}
                        {finding.armor_type && (
                          <div className="bg-black/20 p-1.5 rounded text-[10px] font-mono border border-white/5">
                            <span className="text-muted-foreground uppercase">Armor:</span> {finding.armor_type}
                          </div>
                        )}
                        {finding.country_of_origin && (
                          <div className="bg-black/20 p-1.5 rounded text-[10px] font-mono border border-white/5">
                            <span className="text-muted-foreground uppercase">Origin:</span> {finding.country_of_origin}
                          </div>
                        )}
                        {finding.tracking_id && (
                          <div className="bg-black/20 p-1.5 rounded text-[10px] font-mono border border-white/5">
                            <span className="text-muted-foreground uppercase">Track ID:</span> {finding.tracking_id}
                          </div>
                        )}
                      </div>

                      {finding.reliability_level && (
                        <div className={`text-[10px] font-mono font-bold uppercase p-1.5 rounded text-center mb-3 border ${
                          finding.reliability_level === 'HIGH' ? 'bg-green-500/10 text-green-500 border-green-500/20' :
                          finding.reliability_level === 'MODERATE' ? 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20' :
                          'bg-red-500/10 text-red-500 border-red-500/20'
                        }`}>
                          Reliability: {finding.reliability_level}
                          {finding.reliability_level === 'LOW' && " — REQUIRES HUMAN VERIFICATION"}
                        </div>
                      )}
